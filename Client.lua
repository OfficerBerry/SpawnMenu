...
local MainMenu = UIMenu.New()
local SettingsMenu = UIMenu.New('Settings', 'Settings for the "Add-On Vehicles" menu')
local SubMenus = {}; Items = {}
Pool:Add(MainMenu)
Pool:Add(SettingsMenu)

--SubMenus:MouseControlsEnabled(false)
--SubMenus:MouseEdgeEnabled(false)
--SubMenus:ControlDisablingEnabled(false)
...
function AOVSM.SpawnVehicle(Model)
	...
	if IsModelValid(Model) then
		...
		AddTextComponentString('~r~This spawn code is not working. Either be sure the model is correct or contact the server owner')
